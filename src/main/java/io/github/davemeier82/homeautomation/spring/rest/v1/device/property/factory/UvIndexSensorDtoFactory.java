...
import io.github.davemeier82.homeautomation.spring.rest.v1.device.property.dto.UvSensorPropertyDto;
...
public class UvIndexSensorDtoFactory implements DevicePropertyDtoFactory {

  private final DevicePropertyValueRepository devicePropertyValueRepository;

  public UvIndexSensorDtoFactory(DevicePropertyValueRepository devicePropertyValueRepository) {
  ...
  public DevicePropertyDto map(DeviceProperty deviceProperty) {
    Optional<DataWithTimestamp<Float>> uvIndex = devicePropertyValueRepository.findLatestValue(deviceProperty.getId(), UV_INDEX, Float.class);
    return new UvSensorPropertyDto(
        deviceProperty.getId().id(),
        deviceProperty.getType().getTypeName(),
        deviceProperty.getDisplayName(),
        getValueOrNull(uvIndex),
        getTimestampOrNull(uvIndex)
    );
  }
