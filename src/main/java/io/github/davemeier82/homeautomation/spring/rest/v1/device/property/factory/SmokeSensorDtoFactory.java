...
import io.github.davemeier82.homeautomation.spring.rest.v1.device.property.dto.SmokeSensorPropertyDto;

import java.util.List;
import java.util.Optional;

import static io.github.davemeier82.homeautomation.core.device.property.DefaultDevicePropertyType.SMOKE_SENSOR;
...
public class SmokeSensorDtoFactory implements DevicePropertyDtoFactory {

  private final DevicePropertyValueRepository devicePropertyValueRepository;

  public SmokeSensorDtoFactory(DevicePropertyValueRepository devicePropertyValueRepository) {
  ...
  public DevicePropertyDto map(DeviceProperty deviceProperty) {
    Optional<DataWithTimestamp<Boolean>> isSmoking = devicePropertyValueRepository.findLatestValue(deviceProperty.getId(), SMOKE_STATE, Boolean.class);
    return new SmokeSensorPropertyDto(
        deviceProperty.getId().id(),
        deviceProperty.getType().getTypeName(),
        deviceProperty.getDisplayName(),
        getValueOrNull(isSmoking),
        getTimestampOrNull(isSmoking)
    );
  }
